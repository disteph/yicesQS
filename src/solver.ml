...
module DblCtx = struct
  ...
  let rec check ?param t =
    match Context.check ?param t.cdclT with
    ...
    | `STATUS_SAT ->
       begin
         ...
         match Context.check_with_assumptions t.mcsat assumptions with
         | `STATUS_SAT   ->
...
let treat filename =
  ...
  let set_logic logic _ = print 3 "@[Setting logic to %s@]@," logic in
      ...
      begin
        ...
        | "check-sat", [], _ ->
           ...
           let () =
             ...
             | None -> ()
             | Some model ->
                Format.(fprintf stdout) "@[model %a@]@," (SModel.pp ()) (SModel.make model)
           in
           (* SolverState.free initial_state; *)
           print 3 "@[Context freed@]@,";
