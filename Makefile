...
YICES2_OCAML_BINDINGS ?= ../yices2_ocaml_bindings
YICES_VENDOR_LIBDIR ?= $(abspath $(YICES2_OCAML_BINDINGS)/_build/default/vendor_install/lib)
RUNTIME_LIBRARY_PATHS ?= $(YICES_VENDOR_LIBDIR):$(OPAM_LIBDIR):$(OPAM_STUBLIBS):/usr/local/lib
BV_DELEGATES ?= cadical cryptominisat
REGRESS_SMT2 ?= $(shell git ls-files 'regress/**/*.smt2' 'regress/*.smt2')
REGRESSED_SMT2 ?= $(shell git ls-files 'regressed/**/*.smt2' 'regressed/*.smt2')
REGRESSED_TIMEOUT ?= 60
...
RUN_REGRESSED_EXE = sh -c 'status=0; for file do echo "$$file"; $(RUN_WITH_LIBPATH) timeout $(REGRESSED_TIMEOUT) ./main.exe "$$file" || status=1; done; exit $$status' sh
RUN_REGRESSED_WIDE_PROJECTION = sh -c 'status=0; for spec in "legacy|" "wide=1|-wide-projection 1" "wide=10|-wide-projection 10" "wide=0|-wide-projection 0"; do label="$$(expr "x$$spec" : "x\([^|]*\)")"; flags="$$(expr "x$$spec" : "x[^|]*|\(.*\)")"; echo "Regressed arithmetic projection: $$label"; for file do if grep -Eq "\(set-logic (QF_)?(NRA|LRA|NIA|LIA)\)" "$$file"; then echo "$$file [projection=$$label]"; $(RUN_WITH_LIBPATH) timeout $(REGRESSED_TIMEOUT) ./main.exe $$flags "$$file" || status=1; fi; done; done; exit $$status' sh
...
run-regressed-wide-projection-test: build
	time $(RUN_REGRESSED_WIDE_PROJECTION) $(REGRESSED_SMT2)
